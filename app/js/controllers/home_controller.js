angular.module("app").controller('HomeController', function($scope, $location, socket,$rootScope, $mobileFrame) {
  $scope.rooms = [];
  $scope.chats = {'witlr':[{username:'test',message:'Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna'}], 'comsci':[], 'cardiffuni':[],'test':[]}
  $scope.joined = [];
  ...
  $scope.send = function (room) {
    ...
    $scope.chats[room].push({'username':$rootScope.currentUser, 'message':$scope.currentMsg});
  };
